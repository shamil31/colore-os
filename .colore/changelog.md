...
- Status: DONE
- Evidence: 293 tests passing (26 new, all in `test_security.py`). Doctor's new "Access control" section proves each protected path rejects no-key and wrong-key and accepts the correct key, and that public paths never require one. Verified against the live public server after deploy: `curl https://api.colorebl.com/growth/events/1` — previously `200` with client name, phone and message text — now returns `401 {"detail":"missing or invalid X-Colore-Api-Key"}`; the same request with the correct key returns `200`. `/`, `/docs` unaffected (still `200` anonymous).
